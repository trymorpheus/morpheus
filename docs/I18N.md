...
$crud = new DynamicCRUD($pdo, 'users', locale: 'en');
...
$crud = new DynamicCRUD($pdo, 'users', locale: 'fr');
...
| `validation.email` | The {field} field must be a valid email | El campo {field} debe ser un email válido | Le champ {field} doit être un email valide |
| `validation.url` | The {field} field must be a valid URL | El campo {field} debe ser una URL válida | Le champ {field} doit être une URL valide |
| `validation.number` | The {field} field must be a number | El campo {field} debe ser un número | Le champ {field} doit être un nombre |
| `validation.min` | The {field} field must be at least {min} | El campo {field} debe ser mayor o igual a {min} | Le champ {field} doit être au moins {min} |
| `validation.max` | The {field} field must be at most {max} | El campo {field} debe ser menor o igual a {max} | Le champ {field} doit être au plus {max} |
| `validation.minlength` | The {field} field must be at least {minlength} characters | El campo {field} debe tener al menos {minlength} caracteres | Le champ {field} doit contenir au moins {minlength} caractères |
| `validation.maxlength` | The {field} field cannot exceed {maxlength} characters | El campo {field} no puede exceder {maxlength} caracteres | Le champ {field} ne peut pas dépasser {maxlength} caractères |
...
| `form.select_option` | -- Select an option -- | -- Seleccione una opción -- | -- Sélectionnez une option -- |
| `form.search` | Search... | Buscar... | Rechercher... |
| `form.select_all` | Select All | Seleccionar todo | Tout sélectionner |
...
| Key | English | Spanish | French |
|-----|---------|---------|--------|
| `error.csrf_invalid` | Invalid CSRF token | Token CSRF inválido | Jeton CSRF invalide |
| `error.validation_failed` | Validation failed | Validación fallida | Échec de la validation |
| `success.created` | Record created successfully | Registro creado exitosamente | Enregistrement créé avec succès |
| `success.updated` | Record updated successfully | Registro actualizado exitosamente | Enregistrement mis à jour avec succès |
| `success.deleted` | Record deleted successfully | Registro eliminado exitosamente | Enregistrement supprimé avec succès |

### Many-to-Many UI

| Key | English | Spanish | French |
|-----|---------|---------|--------|
| `m2m.selected` | {count} selected | {count} seleccionados | {count} sélectionnés |
| `m2m.no_results` | No results found | No se encontraron resultados | Aucun résultat trouvé |
...
return [
    ...
    'validation.email' => 'Das Feld {field} muss eine gültige E-Mail sein',
    'validation.url' => 'Das Feld {field} muss eine gültige URL sein',
    'validation.number' => 'Das Feld {field} muss eine Zahl sein',
    'validation.min' => 'Das Feld {field} muss mindestens {min} sein',
    'validation.max' => 'Das Feld {field} darf höchstens {max} sein',
    ...
    'form.select_option' => '-- Wählen Sie eine Option --',
    'form.search' => 'Suchen...',
    'form.select_all' => 'Alle auswählen',
    'form.clear_all' => 'Alle löschen',
    ...
    'success.deleted' => 'Datensatz erfolgreich gelöscht',
    
    // Many-to-many
    'm2m.selected' => '{count} ausgewählt',
...
```php
$crud = new DynamicCRUD($pdo, 'users', locale: 'de');
echo $crud->renderForm();
```
...
<body>
    <div class="language-switcher">
        <a href="?lang=en" <?= $currentLocale === 'en' ? 'class="active"' : '' ?>>🇬🇧 English</a>
        <a href="?lang=es" <?= $currentLocale === 'es' ? 'class="active"' : '' ?>>🇪🇸 Español</a>
        <a href="?lang=fr" <?= $currentLocale === 'fr' ? 'class="active"' : '' ?>>🇫🇷 Français</a>
    </div>
